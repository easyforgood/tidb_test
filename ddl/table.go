...
func onCreateTable(d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, _ error) {
	if vmockExceedErrorLimit, __fpErr := __fp_mockExceedErrorLimit.Acquire(); __fpErr == nil { defer __fp_mockExceedErrorLimit.Release(); mockExceedErrorLimit, __fpTypeOK := vmockExceedErrorLimit.(bool); if !__fpTypeOK { goto __badTypemockExceedErrorLimit} 
		 if mockExceedErrorLimit {
		    return ver, errors.New("mock do job error")
		 }; __badTypemockExceedErrorLimit: __fp_mockExceedErrorLimit.BadType(vmockExceedErrorLimit, "bool"); };
